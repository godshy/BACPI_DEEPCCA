...
class GATLayer(nn.Module):
    ...
    def forward(self, h, adj):
        ...
        attention = torch.where(adj > 0, e, zero_vec)  # either e or replaced by zero_vec if element in adj is less than 0
...
#            params['window'], params['layer_cnn'], params['latent_dim'], params['layer_out']
    ...
    def comp_gat(self, atoms, atoms_mask, adj):
        ...
        print('atoms_vector.shape',  atoms_vector.shape)
        return atoms_vector
...
class BACPI_DEEPCCA(nn.Module):
    ...
    def bidirectional_attention_prediction(self, atoms_vector, atoms_mask, fps, amino_vector, amino_mask, new_feature):
        b = atoms_vector.shape[0]  # here b== batch_size
        ...
        # print('pf_final_shape:', pf_final.view(b, 1, -1).shape)
        cf_pf = F.leaky_relu(torch.matmul(cf_final_1.view(b, -1, 1), pf_final.view(b, 1, -1)).view(b, -1), 0.1)
        # print('cf_pf before leaky shape:', torch.matmul(cf_final_1.view(b, -1, 1), pf_final.view(b, 1, -1)).view(b, -1).shape)
        return self.output(cf_pf)

    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps, new_feature):
        atoms_vector = self.comp_gat(atoms, atoms_mask, adjacency)
        amino_vector = self.prot_cnn(amino, amino_mask)
        ...
        prediction = self.bidirectional_attention_prediction(atoms_vector, atoms_mask, super_feature, amino_vector, amino_mask, new_feature)
        return prediction

#### Delete ECFP in training process

class BACPI_DEEPCCA_NOECFP(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        super(BACPI_DEEPCCA_NOECFP, self).__init__()
        ...
        # need to change to fit new feature space 12800->13600
        if task == 'affinity':
            self.output = nn.Linear(latent_dim * (latent_dim - 35) * 2, 1)
        elif task == 'interaction':
            self.output = nn.Linear(latent_dim * (latent_dim - 35) * 2, 2)
        else:
            print("Please choose a correct mode!!!")
...
class BACPI_DEEPCCA_NOECFP_DIM20(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        super(BACPI_DEEPCCA_NOECFP_DIM20, self).__init__()
        ...
        # need to change to fit new feature space 12800->13600
        if task == 'affinity':
            self.output = nn.Linear(latent_dim * (latent_dim - 30) * 2, 1)
        elif task == 'interaction':
            self.output = nn.Linear(latent_dim * (latent_dim - 30) * 2, 2)
        else:
            print("Please choose a correct mode!!!")
...
class BACPI_DEEPCCA_NOECFP_DIM30(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        super(BACPI_DEEPCCA_NOECFP_DIM30, self).__init__()
        ...
        # need to change to fit new feature space 12800->13600
        if task == 'affinity':
            self.output = nn.Linear(latent_dim * (latent_dim - 25) * 2, 1)
        elif task == 'interaction':
            self.output = nn.Linear(latent_dim * (latent_dim - 25) * 2, 2)
        else:
            print("Please choose a correct mode!!!")
...
class BACPI_DEEPCCA_NOECFP_DIM3(nn.Module):
    def __init__(self, task, n_atom, n_amino, params):
        super(BACPI_DEEPCCA_NOECFP_DIM3, self).__init__()
        ...
        # need to change to fit new feature space 12800->13600
        if task == 'affinity':
            self.output = nn.Linear(latent_dim * (latent_dim + 3), 1)
        elif task == 'interaction':
            self.output = nn.Linear(latent_dim * (latent_dim + 3), 2)
        else:
            print("Please choose a correct mode!!!")
